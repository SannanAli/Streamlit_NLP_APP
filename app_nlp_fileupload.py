...
# PDF Reader Method
def loadPDF(PDFfile):
    pdf_file = PdfReader(PDFfile)
    count = len(pdf_file.pages)
    all_pages_text = ""
    for i in range(count):
        pages = pdf_file.pages[i]
...
def fileupload():
    ...
    raw_text = st.file_uploader(
        "Upload Your Document Here", type=["pdf", "docx", "txt"]
    )

    if st.button("Analyze"):
        if raw_text is not None:
            if raw_text.type == "application/pdf":
                st.subheader(raw_text.name)
                raw_text = loadPDF(raw_text)
                # st.write(pdf_file)

            elif raw_text.type == "text/plain":
                st.subheader(raw_text.name)
                raw_text = str(raw_text.read(), "utf-8")
                # st.write(raw_text)

            else:
                st.subheader(raw_text.name)
                raw_text = docx2txt.process(raw_text)
                # st.write(docx_text)
            ...
            with c2:
                with st.expander("Plot Word Frequency"):
                    fig = plt.figure()
                    Top_keywords = get_most_common_tokens(process)
